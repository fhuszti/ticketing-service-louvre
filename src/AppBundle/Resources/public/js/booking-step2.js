$(function() {
    ...
    function addTicketForm(collectionHolder) {
        ...
        var newForm = prototype.replace(/__name__label__/g, Translator.trans('core.step2.ticket.label', { "index": indexUp }))
        ...
        collectionHolder.append($('<div class="row"></div>')
                            .append(newForm)
                            .append($('<div class="col-xs-12 col-sm-6 col-sm-pull-6"><h3>'+Translator.trans('core.step2.price.label')+' <span id="price_'+index+'">-</span></h3></div>'))
                        )
                        .append($('<hr />'));
    }
    ...
    function designTicketForm(collectionHolder) {
        ...
        rowDiv.children('div').children('label').text( Translator.trans('core.step2.ticket.label', { "index": indexUp }) );
        rowDiv.append( $('<div class="col-xs-12 col-sm-6 col-sm-pull-6"><h3>'+Translator.trans('core.step2.price.label')+' <span id="price_'+index+'">-</span></h3></div>') );
    ...
    function addBootstrapClasses(container) {
        var containerId = container.attr('id'),
            formControlDivs = $('div[id^="appbundle_ticketorder_tickets_"] div:not([class*="alert alert-danger"])'),
            reducedPriceDivs = $('div[id^="appbundle_ticketorder_tickets_"] div:nth-child(5)');
    ...
    function generateTicketForms() {
        var i, j,
            collectionHolder = $('#appbundle_ticketorder_tickets'),
            nbTickets = $('#appbundle_ticketorder_nbTickets').val();
        
        collectionHolder.data('index', 0);

        if ( $('#appbundle_ticketorder_tickets_0').length <= 0 ) {
    ...
    function formatDate(dateElmt, index) {
        ...
        if (dateString.split('-').length > 1) {
            var convertedDate = convertDatePhpToFrench(dateString),
                phpDateElmt = $('#appbundle_ticketorder_tickets_'+index+'_php_birthDate');
    ...
    function managePriceOnDateChange(dateText, currentIndex, specialRate) {
        var price = getPriceFromDate(dateText),
            ...
            priceElmt = $('#appbundle_ticketorder_tickets_'+currentIndex+'_price'),
            ticketTypeField = $('#appbundle_ticketorder_type'),
            discountedInput = $('#appbundle_ticketorder_tickets_'+currentIndex+'_discounted');
     ...
     function manageSpecialRateCheckboxOnDateChange(birthdayDate, currentIndex) {
        var dates = getUsefulDates(birthdayDate),
            checkbox = $('#appbundle_ticketorder_tickets_'+currentIndex+'_discounted');
    ...
    function managePriceOnSpecialRateChange(specialRate, checkboxElement) {
        var splitFieldId = checkboxElement.attr('id').split('_'),
            ...
            dateText = $('#appbundle_ticketorder_tickets_'+currentIteration+'_birthDate').val(),
            price = specialRate ? 10 : (dateText === '' ? 0 : getPriceFromDate(dateText)),
            priceSpan = $('#price_'+currentIteration),
            priceElmt = $('#appbundle_ticketorder_tickets_'+currentIteration+'_price'),
            ticketTypeField = $('#appbundle_ticketorder_type');
    ...
    function updateTotalPrice() {
        ...
        $('#appbundle_ticketorder_price').val(totalPrice);
    }
        ...
        for (var i = 0; i < dateInputs.length; i++) {
            if (dateInputs[i]) {
                ...
                $(dateInputs[i]).datepicker({
                    ...
                    yearRange: "-120:+0",
                    dateFormat: "dd/mm/yy",
                    altField: '#appbundle_ticketorder_tickets_'+i+'_php_birthDate',
                    altFormat: "yy-mm-dd",
